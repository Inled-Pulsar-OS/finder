  ...
  if (before && before_section_type != row_section_type)
    {
      switch (row_section_type)
        {
          case NAUTILUS_GTK_PLACES_SECTION_DEFAULT_LOCATIONS:
          case NAUTILUS_GTK_PLACES_SECTION_BOOKMARKS:
            title = _("Favorites");
            break;
          case NAUTILUS_GTK_PLACES_SECTION_CLOUD:
            title = _("iCloud & Cloud Drives");
            break;
          case NAUTILUS_GTK_PLACES_SECTION_MOUNTS:
            title = _("Locations");
            break;
          case NAUTILUS_GTK_PLACES_SECTION_TAGS:
            title = _("Tags");
            break;
          default:
            break;
      ...
      if (title != NULL)
        {
          ...
          gtk_widget_add_css_class (header_box, "sidebar-section-header");
          ...
          gtk_list_box_row_set_header (row, header_box);
        }
      else
        {
...
update_places (NautilusGtkPlacesSidebar *sidebar)
{
  ...
  /* add built-in places - Finder Favorites style */

  /* Recent Files (Top of Finder Favorites) */
  if (should_show_recent (sidebar))
    {
      ...
      add_place (sidebar, NAUTILUS_GTK_PLACES_BUILT_IN,
                 ...
                 _("Recents"), start_icon, NULL, SCHEME_RECENT ":///",
  ...
  /* Cloud Providers & Cloud Folders (Google Drive, OneDrive, iCloud) */
  {
    ...
    struct {
      ...
      { "iCloud", "iCloud Drive", "folder-remote-symbolic" },
      { "iCloud Drive", "iCloud Drive", "folder-remote-symbolic" },
      { "Google Drive", "Google Drive", "folder-remote-symbolic" },
      { "GoogleDrive", "Google Drive", "folder-remote-symbolic" },
      { "OneDrive", "OneDrive", "folder-remote-symbolic" },
      { "Nextcloud", "Nextcloud", "folder-remote-symbolic" },
      { "Dropbox", "Dropbox", "folder-remote-symbolic" },
    };

    for (guint c_idx = 0; c_idx < G_N_ELEMENTS (cloud_candidates); c_idx++)
      {
        g_autofree char *c_path = g_build_filename (home_dir, cloud_candidates[c_idx].rel_path, NULL);
        if (g_file_test (c_path, G_FILE_TEST_IS_DIR))
          {
            g_autofree char *c_uri = g_filename_to_uri (c_path, NULL, NULL);
            GIcon *c_icon = g_themed_icon_new_with_default_fallbacks (cloud_candidates[c_idx].icon_name);
            add_place (sidebar, NAUTILUS_GTK_PLACES_BUILT_IN,
                       NAUTILUS_GTK_PLACES_SECTION_CLOUD,
                       cloud_candidates[c_idx].name, c_icon, NULL, c_uri,
                       NULL, NULL, NULL, NULL, 0,
                       cloud_candidates[c_idx].name);
